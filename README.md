...
- Driveの特定フォルダ配下の **ファイル/フォルダ一覧** を表示
- ファイルをクリックすると右側に **プレビュー表示**
- フォルダをクリックするとそのフォルダに移動（戻るボタンあり）
- 「フォルダを選ぶ」から **フォルダツリー** で選択（マイドライブ / 共有ドライブ / 共有アイテム）
- フォルダ読み込み後、入力欄右の **「☆」** でお気に入り登録/解除（**即保存**）
- サイドバーの **「お気に入り」** はショートカット（折りたたみ可）
- お気に入りの追加/削除は **設定（⚙）内の「お気に入り」** で行えます（即保存）
- 設定（お気に入り、ソート、件数制限、履歴、テーマ）は **ユーザーごと** に保存（`UserProperties`）
...
- `Drive.Drives.list()` を使うため、アカウント種別/権限によっては取得できません  
  - この場合、UIは落とさず空として扱う設計です（エラー文がツリー内に出ることがあります）
...
- `Index.html` は `google.script.run` で `Code.gs` の関数を呼び出します（Webアプリとしての実行が前提）
- `Index.html` 冒頭付近にローカル向けのログ送信先が埋め込まれています（`127.0.0.1`）。失敗しても握りつぶすため通常利用では影響しません。気になる場合は該当の `LOG_ENDPOINT` 周りを削除/無効化してください（挙動変更になるので、運用前にテスト推奨）。
